...
class Rule:
    ...
    def CanHu(hand: list[Tile]) -> tuple[bool, list[Meld]]:
        ...
        """
        ValidLen = [i for i in range(Rule.PairLen, Rule.HandLen+1, Rule.MeldLen)]
        ...
        for tile in HandCounts:
            # 假設為對牌
            if HandCounts[tile] >= Rule.PairLen:
                # 複製一份手牌來操作，避免影響原始資料
                TempCounts = HandCounts.copy()

                # 移除眼睛
                TempCounts[tile] -= Rule.PairLen
                if TempCounts[tile] == 0:
                    del TempCounts[tile]

                # 3. 遞迴拆解剩下的 15 張牌
                if Rule.CanBeMelds(TempCounts, melds):
                    #新增對子
    ...
    def CanBeMelds(hand: Counter, melds:list[Meld]) -> bool:
        ...
        if hand[FirstTile] >= 3:
            ...
            # 遞迴移除刻子
            if Rule.CanBeMelds(hand.copy(), melds): # 傳遞副本
                #新增刻子
        ...
        # 順子:7,8,9
        if FirstTile.Suit != SUIT.HONOR and FirstTile.Suit != SUIT.FLOWER and FirstTile.Num <= Rule.NumHiLimit:
